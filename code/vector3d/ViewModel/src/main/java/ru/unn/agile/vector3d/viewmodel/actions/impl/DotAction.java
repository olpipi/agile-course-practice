...
public class DotAction implements IAction {
    @Override
    public void execute(final ViewModel viewModel) {
        ...
        Vector3D other = createVectorFromStrings(viewModel.getOtherVectorX(),
                viewModel.getOtherVectorY(), viewModel.getOtherVectorZ());

        viewModel.resultProperty().set(String.valueOf(vector.dot(other)));
